...
type PlanExecutor interface {
	...
	PrepareShuffle(part ReduceablePartition, buckets []uint64) error                                                                                                                                 // PrepareShuffle appropriately caches and sorts a Partition before making it available for shuffling
	...
	ShufflePartitionData(wg *sync.WaitGroup, partMerger chan<- ReduceablePartition, asyncErrors chan<- error, mpart *pb.MPartitionMeta, dataStream pb.PartitionsService_TransferPartitionDataClient) // AcceptShuffledPartition receives a Partition that belongs on this worker and merges it into the local shuffle tree
	MergeShuffledPartitions(wg *sync.WaitGroup, partMerger <-chan ReduceablePartition, asyncErrors chan<- error)                                                                                     // MergeShuffledPartitions activates this PlanExecutor's shuffled partition merger, until the given channel is closed

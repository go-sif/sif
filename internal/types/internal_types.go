...
type CloneablePartition interface {
	...
	GetCurrentSchema() types.Schema                       // GetCurrentSchema retrieves the Schema from the most recent task that manipulated this Partition
	GetWidestSchema() types.Schema                        // GetWidestSchema retrieves the widest Schema from the stage that produced this Partition, which is equal to the size of a row
...
type ReduceablePartition interface {
	CloneablePartition
	types.BuildablePartition
	types.KeyablePartition
	FindFirstKey(key uint64) (int, error)                                                // PRECONDITION: Partition must already be sorted by key
	FindFirstRowKey(keyBuf []byte, key uint64, keyfn types.KeyingOperation) (int, error) // PRECONDITION: Partition must already be sorted by key
...
type TransferrablePartition interface {
	...
	ReceiveStreamedData(stream pb.PartitionsService_TransferPartitionDataClient, incomingSchema types.Schema) error // ReceiveStreamedData loads data from a protobuf stream into this Partition
}
